...
struct MultiplicityCounter {
  ...
  if (doprocessMCMult) {
      registry.add({"Tracks/ProcessMCMult/hdndeta_midrapidity", "evntclass;  zvtex, eta", {HistType::kTHnSparseD, {ZAxis, EtaAxis}}});
      registry.add({"Tracks/ProcessMCMult/hdndeta_mftrapidity", "evntclass;  zvtex, eta", {HistType::kTHnSparseD, {ZAxis, EtaAxis}}});
    }
  ...
  void processMCMult(
    ...
    // Particles const& mcparticles,
    aod::McParticles const& mcparticles,
    // soa::Filtered<LabeledTracksEx> const& mctracks, // filter 때문에 에러 발생
    soa::Filtered<soa::Join<aod::Tracks, aod::McTrackLabels, aod::TracksExtra, aod::TrackSelection, aod::TracksDCA>> const& mctracks,
    // LabeledTracksEx const& mctracks,
    // DaughterTracks const&, 
    // soa::SmallGroups<aod::ReassignedTracksCore> const& atracks생
  ...
  //   aod::McCollisions const& mccollisions,
  //   soa::Join<aod::Tracks, aod::McTrackLabels, aod::TracksExtra, aod::TrackSelection, aod::TracksDCA> const& mctracks,
  //   aod::McParticles const& mcparticles,
  //   soa::Join<aod::MFTTracks, aod::McMFTTrackLabels> const& mcmfttracks
  // )
  {
    for(auto& collision : collisions)
    {
      ...
      for(auto& mctrack : permctracks)
      {
        ...
        registry.fill(HIST("Tracks/ProcessMCMult/hdndeta_midrapidity"), z, mctrack.mcParticle_as<aod::McParticles>().eta());
      }
      ...
      for(auto& mcmfttrack : permcmfttracks)
      {
        ...
        registry.fill(HIST("Tracks/ProcessMCMult/hdndeta_mftrapidity"), z, mcmfttrack.mcParticle_as<aod::McParticles>().eta());
      }
      }
