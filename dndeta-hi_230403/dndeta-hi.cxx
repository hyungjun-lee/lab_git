...
struct MultiplicityCounter {
  ...
  void init(InitContext&)
  {
    ...
    if (doprocessMCMult) {
      registry.add({"Tracks/ProcessMCMult/hdndeta", "evntclass;  zvtex, eta", {HistType::kTHnSparseD, {testAxis, ZAxis, EtaAxis}}});
      registry.add({"Tracks/ProcessMCMult/hzvtx", "zvtex", {HistType::kTHnSparseD, {ZAxis}}});
    }
    if (doprocessDATAMult) {
      registry.add({"Tracks/ProcessDATAMult/hdndeta", "evntclass;  zvtex, eta", {HistType::kTHnSparseD, {testAxis, ZAxis, EtaAxis}}});
      registry.add({"Tracks/ProcessDATAMult/hzvtx", "zvtex", {HistType::kTHnSparseD, {ZAxis}}});
    }
    if (doprocessMultTest) {
      registry.add({"Tracks/ProcessMultTest/hgen", "zvtex, mult", {HistType::kTHnSparseD, {ZAxis, MultAxis}}});
      registry.add({"Tracks/ProcessMultTest/hrec", "zvtex, mult", {HistType::kTHnSparseD, {ZAxis, MultAxis}}});
      registry.add({"Tracks/ProcessMultTest/size", "zvtex, mult", {HistType::kTHnSparseD, {testAxis}}});
      registry.add({"Tracks/ProcessMultTest/response", "zvtex, mult", {HistType::kTHnSparseD, {MultAxis,MultAxis}}});
    }
  }
  ...
  void processMCMult(
    soa::Join<aod::Collisions, aod::McCollisionLabels, aod::EvSels> const& collisions,
    aod::McCollisions const& mccollisions,
    soa::Filtered<soa::Join<aod::Tracks, aod::McTrackLabels, aod::TracksExtra, aod::TrackSelection, aod::TracksDCA>> const& mctracks,
    aod::McParticles const& mcparticles,
    soa::Join<aod::MFTTracks, aod::McMFTTrackLabels> const& mcmfttracks)
  {
    for (auto& collision : collisions) {
      ...
      auto z = collision.posZ();
      registry.fill(HIST("Tracks/ProcessMCMult/hzvtx"), z);
      auto permctracks = mctracks.sliceBy(mctracks_slice, collision.globalIndex());
      auto permcmfttracks = mcmfttracks.sliceBy(mcmfttracks_slice, collision.globalIndex());
      permctracks.bindExternalIndices(&mcparticles);

      for (auto& mctrack : permctracks) {
        if (!mctrack.has_mcParticle()) {
          continue;
        }
        registry.fill(HIST("Tracks/ProcessMCMult/hdndeta"), 1, z, mctrack.mcParticle_as<aod::McParticles>().eta());
      }

      for (auto& mcmfttrack : permcmfttracks) {
        if (!mcmfttrack.has_mcParticle()) {
          continue;
        }
        registry.fill(HIST("Tracks/ProcessMCMult/hdndeta"), 2, z, mcmfttrack.mcParticle_as<aod::McParticles>().eta());
      }
    }
  }
  PROCESS_SWITCH(MultiplicityCounter, processMCMult, "Process dndeta-with mft track", false);
  ...
  void processDATAMult(
    soa::Join<aod::Collisions, aod::EvSels> const& collisions,
    soa::Filtered<soa::Join<aod::Tracks, aod::TracksExtra, aod::TrackSelection, aod::TracksDCA>> const& tracks,
    aod::MFTTracks const& mfttracks)
  {
    for (auto& collision : collisions) {
      ...
      auto z = collision.posZ();
      registry.fill(HIST("Tracks/ProcessDATAMult/hzvtx"), z);
      auto pertracks = tracks.sliceBy(tracks_slice, collision.globalIndex());
      auto permfttracks = mfttracks.sliceBy(mfttracks_slice, collision.globalIndex());

      for (auto& track : pertracks) {
        registry.fill(HIST("Tracks/ProcessDATAMult/hdndeta"), 1, z, track.eta());
      }

      for (auto& mfttrack : permfttracks) {
        registry.fill(HIST("Tracks/ProcessDATAMult/hdndeta"), 2, z, mfttrack.eta());
      }
    }
  }
  PROCESS_SWITCH(MultiplicityCounter, processDATAMult, "Process dndeta-with mft track", false);

  Partition<Particles> mcSample2 = (aod::mcparticle::eta < 5.f) && (aod::mcparticle::eta > 2.2f);
  
  void processMultTest(
    aod::McCollisions::iterator const& mcCollision,
    o2::soa::SmallGroups<soa::Join<aod::Collisions, aod::EvSels, aod::McCollisionLabels>> const& collisions,
    Particles const& mcparticles,
    soa::Join<FiTracks, aod::McTrackLabels> const& mctracks)
  {
    // generated particle tagging
    auto perCollisionMCSample = mcSample2->sliceByCached(aod::mcparticle::mcCollisionId, mcCollision.globalIndex()); // auto nCharged = 0;
    auto nCharged = 0;
    for (auto& particle : perCollisionMCSample) {
      auto charge = 0.;
      auto p = pdg->GetParticle(particle.pdgCode());
    ...
    registry.fill(HIST("Tracks/ProcessMultTest/hgen"), mcCollision.posZ(), nCharged);

    // auto permccol = mcparticles.sliceBy(mcparticle_slice, mcCollision.globalIndex());
    // auto nGen_Charge = 0;
    // for (auto& mcparticle : permccol) {
    // auto pid = pdg->GetParticle(mcparticle.pdgCode());
    // if (pid == nullptr) {
    //   continue;
    // }
    // auto charge = pid->Charge();
    // if (std::abs(charge) > 3.) {
    //   nGen_Charge++;
    // }
    // }
    ...
    auto nRec = 0;
    // reconstruction particle tagging
    for (auto& collision : collisions) {
    auto percol = mctracks.sliceBy(mctracks_slice, collision.globalIndex());
    registry.fill(HIST("Tracks/ProcessMultTest/size"), percol.size());

    if (useEvSel && !collision.sel8()) { // event selection cut
      continue;
    }

    for (auto& mctrack : percol) {
      if (!mctrack.has_mcParticle()) {
        continue;
      }
      if(abs(mctrack.eta())>0.5)
      {
        continue;
      }
      nRec++;
    }
    registry.fill(HIST("Tracks/ProcessMultTest/hrec"), collision.posZ(), nRec);
    registry.fill(HIST("Tracks/ProcessMultTest/response"), nCharged, nRec);
  ...
  PROCESS_SWITCH(MultiplicityCounter, processMultTest, "Process generator-level info", false);
};
